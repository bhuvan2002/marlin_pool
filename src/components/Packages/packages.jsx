  ...
  const handleAddClick = () => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      if (editIndex !== null) {
        ...
        updatedPackages[editIndex] = newPackage;
      ...
      setPreviewVisible(true); // Show preview dialog immediately after adding
    ...
    <div className="container">
      ...
      <Dialog header="Preview Package" visible={previewVisible} style={{ width: '50vw' }} onHide={() => setPreviewVisible(false)}>
        <div className="fade-in">
          ...
          {hasDiscount && <p><strong>Discount:</strong> {discount}%</p>}
        </div>
      </Dialog>
      ...
      <div className="package-list">
        {packages.map((pkg, index) => (
          <div key={index} className="package-item">
            ...
            <p><strong>Cost:</strong> Rs {pkg.cost}</p>
            <p><strong>Duration:</strong> {pkg.duration} months</p>
            {pkg.hasDiscount && <p><strong>Discount:</strong> {pkg.discount}%</p>}
            <Button label="Edit" onClick={() => handleEdit(index)} className="button-secondary" />
